...
class WalletTest(FujicoinTestFramework):
    ...
    def run_test(self):
        ...
        # node0 should end up with 100 fjc in block rewards plus fees, but
        ...
        fee_rate_sat_vb = 2
        fee_rate_fjc_kvb = fee_rate_sat_vb * 1e3 / 1e8
        explicit_fee_rate_fjc_kvb = Decimal(fee_rate_fjc_kvb) / 1000
        ...
        node_2_bal = self.check_fee_amount(balance, node_2_bal - Decimal('10'), explicit_fee_rate_fjc_kvb, self.get_vsize(self.nodes[2].gettransaction(txid)['hex']))
        ...
        self.sync_all(self.nodes[0:3])
        balance = self.nodes[2].getbalance()
        node_2_bal = self.check_fee_amount(balance, node_2_bal - amount, explicit_fee_rate_fjc_kvb, self.get_vsize(self.nodes[2].gettransaction(txid)['hex']))
        assert_equal(balance, node_2_bal)
        node_0_bal += amount
        assert_equal(self.nodes[0].getbalance(), node_0_bal)
        ...
        for target, mode in product([-1, 0], ["fjc/kb", "sat/b"]):
        ...
        if not self.options.descriptors:
            ...
            assert_fee_amount(fee, tx_size, Decimal(fee_rate_fjc_kvb))
