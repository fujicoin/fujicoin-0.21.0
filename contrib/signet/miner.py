...
def next_block_delta(last_nbits, last_hash, ultimate_target, do_poisson):
    ...
    INTERVAL = 60.0*2016/2015 # 1 minutes, adjusted for the off-by-one bug
...
def fujicoin_cli(basecmd, args, **kwargs):
    cmd = basecmd + ["-signet"] + args
    logging.debug("Calling fujicoin-cli: %r", cmd)
...
def main():
    ...
    parser.add_argument("--cli", default="fujicoin-cli", type=str, help="fujicoin-cli command")
    ...
    args.bcli = lambda *a, input=b"", **kwargs: fujicoin_cli(args.cli.split(" "), list(a), input=input, **kwargs)
